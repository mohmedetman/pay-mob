...
class PaymentController extends Controller
{
    ...
    public function createStaticPayment()
    {
        try {
            $authToken = $this->paymobService->authenticate();
            $orderData = $this->paymobService->registerOrder($authToken);
            $paymentKey = $this->paymobService->getPaymentKey($authToken, $orderData['id']);
